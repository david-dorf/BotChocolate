...
class MoveBot(Node):
    ...
    async def plan_callback(self, request, response):
        ...
        if self.state == State.CART_MSG:
            ...
            self.cart_response = await self.cart_client.call_async(
                GetCartesianPath.Request(
                    header=plan_msg.cart_request.header,
            ...
            if self.state == State.CART_MSG:
                ...
                self.get_logger().info("CHANGING TO CART EXEC STATE")
        ...
        if self.state == State.HOME_MSG:
            ...
            goal_in_joint_config = RobotState()
            goal_in_joint_config.joint_state.position = (
                self.joint_statesmsg.position
            )  # GOAL POS IS JUST JOINT STATES
            plan_msg = self.get_motion_request(
                start_in_joint_config, goal_in_joint_config, request.execute_now
            )
